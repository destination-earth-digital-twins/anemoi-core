...
class AnemoiTrainer:
    ...
    def get_processed_configs(self, config) -> DictConfig:
        ...
        args:
            config: OmegaConf.DictConfig - base configuration
        return:
    ...
    def datamodule(self) -> Any:
        ...
        LOGGER.info(
            "Number of data variables: %s",
            str(
                len(datamodule.data_variables)
            ),  # str(len(datamodule.ds_train.data.variables))
        )
        LOGGER.info(
            "Variables: %s", str(datamodule.data_variables)
        )  # str(datamodule.ds_train.data.variables))
        return datamodule
