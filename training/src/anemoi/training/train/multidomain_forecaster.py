...
class MultiDomainGraphForecaster(pl.LightningModule):
    """Multi domain Graph neural network forecaster for PyTorch Lightning."""
        ...
        **kwargs,
        ...
        if isinstance(config_container, list):
            return torch.nn.ModuleList(
                [
                    MultiDomainGraphForecaster.get_loss_function(
    ...
    def rollout_step(
        ...
        msg = (
            ...
            f", {batch_data.shape[1]} !>= {rollout + self.multi_step}"
        )
    ...
    def _step(
        ...
        for loss_next, metrics_next, y_preds_next in self.rollout_step(
            (batch_data, graph_label[0]), # TODO: previously graph_label[0] list of labels when bs > 1
    ...
    def allgather_batch(self, batch: tuple[torch.Tensor, list]) -> tuple[torch.Tensor, list]:
        ...
        shard_shape = list(batch_data.shape)
        shard_shape[-2] = grid_shard_size
        last_shard_shape = list(batch_data.shape)
    ...
    def validation_step(self, batch: torch.Tensor, batch_idx: int) -> None:
        ...
        None
        """
        # TODO: this will not work for default NativeGridDataset (only yields tensor, not tensor and label)
        batch_data, graph_label = batch
        with torch.no_grad():
